...
def make_preservationxml(aip_id, aip_title, department, workflow, log_path):
    ...
    if file_count == 1:
        stylesheet = f'{stylesheets}/fits-to-preservation_singlefile.xsl'
    elif file_count > 1:
        stylesheet = f'{stylesheets}/fits-to-preservation_multifile.xsl'
    else:
    ...
    subprocess.run(
        f'java -cp {saxon} net.sf.saxon.Transform -s:"{cleaned_fits}" -xsl:"{stylesheet}" -o:"{preservation_xml}" '
    ...
    validation = subprocess.run(f'xmllint --noout -schema "{stylesheets}/preservation.xsd" "{preservation_xml}"',
