...
class TestMakeBag(unittest.TestCase):
    ...
    def test_av_bmac(self):
        ...
        aips_dir = os.path.join(os.getcwd(), 'make_bag')
        aip = AIP(aips_dir, 'bmac', 'mp4', 'rabbitbox', 'folder', 'av', 'rabbitbox_0001', 'title', 'InC', 1, True)
    ...
    def test_av_russell(self):
        ...
        # A copy of the AIP is made since this test should move it to an error folder.
        aips_dir = os.path.join(os.getcwd(), 'make_bag')
        aip = AIP(aips_dir, 'russell', 'mp4', 'rbrl025', 'folder', 'av', 'rbrl025_0001_media', 'title', 'InC', 1, True)
        shutil.copytree(os.path.join(aips_dir, f'{aip.id}_copy'), os.path.join(aips_dir, aip.id))
        make_bag(aip)
    ...
    def test_general(self):
        ...
        # A copy of the AIP is made since this test should move it to an error folder.
        aips_dir = os.path.join(os.getcwd(), 'make_bag')
        aip = AIP(aips_dir, 'russell', None, 'rbrl_025', 'folder', 'general', 'rbrl_025_er_000001', 'title', 'InC', 1, True)
        shutil.copytree(os.path.join(aips_dir, f'{aip.id}_copy'), os.path.join(aips_dir, aip.id))
        make_bag(aip)
    ...
    def test_temp(self):
        ...
        # A copy of the AIP is made since this test should move it to an error folder.
        aips_dir = os.path.join(os.getcwd(), 'make_bag')
        aip = AIP(aips_dir, 'test', None, 'test_001', 'folder', 'general', 'test_001_01', 'title', 'InC', 1, True)
        shutil.copytree(os.path.join(aips_dir, f'{aip.id}_copy'), os.path.join(aips_dir, aip.id))
        make_bag(aip)
